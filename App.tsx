...
const passwordShema = yup.object().shape({
  passwordLength: yup.number()
    .min(4, 'Should be min of 4 charachters')
...
export default function App() {
  ...
  const generatedPasswordString = (passwordLength: number) => {
    ...
    if (upperCase) {
      charachtersList += upperCaseChars
    }
    if (lowerCase) {
      charachtersList += lowerCaseChars
    }
    if (number) {
      charachtersList +=  digitChars
    }
    if (symbols) {
      charachtersList += specialChars
    }
    const passwordResult = createPassword(charachtersList, passwordLength)
